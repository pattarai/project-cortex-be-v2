...
export default class CompleteProfile {
    ...
    public completeProfileGet = async (req: Request, res: Response): Promise<any> => {
        try {
    ...
    public completeProfilePatch = async (req: Request, res: Response): Promise<any> => {
        try {
            const { userId, dateOfBirth, collegeName, department, year, rollNumber, registerNumber, whatsappNumber, instagramUrl, githubUrl, linkedInUrl, description } = req.body
